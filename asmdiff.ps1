...
if (-Not (Test-Path ".\pokemaragda.gba"))
{
    Write-Host "File 'pokemaragda.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokemaragda.gba > .\pokemaragda.dump
Invoke-Expression "$DiffTool .\baserom.dump .\pokemaragda.dump"
